...
export default function ProductosSegundaMano() {
        ...
        const loadProducts = async () => {
            try {
                ...
                if (firebaseProducts && firebaseProducts.length > 0) {
                    ...
                    const segundaManoProductos = firebaseProducts.filter(
                        (producto) => producto.category === 'segunda-mano'
                    );

                    // Formatear productos con precio
                    const formattedProductos = segundaManoProductos.map(
                        (producto) => ({
                            ...producto,
                    ...
                    setFilteredProductos(formattedProductos);
                } else {
        ...
        if (sortKey) {
            filtered.sort((a, b) => {
                ...
                if (sortKey.includes('price')) {
                    ...
                    aValue = a.name;
                    bValue = b.name;
                } else if (sortKey.includes('condition')) {
                    const conditionOrder = {
                        excelente: 1,
                        bueno: 2,
                        regular: 3,
                    };
                    aValue = conditionOrder[a.condition] || 4;
                    bValue = conditionOrder[b.condition] || 4;
                }
    ...
    // Función para obtener severidad de condición
        ...
        <div className="w-full max-w-none py-8 pt-28 px-4 sm:px-6 lg:px-8 min-h-screen">
            <div className="mb-8">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400 transition-colors duration-300">
                    Encuentra cajas fuertes y sistemas de seguridad de segunda
                    mano en excelente estado. Calidad garantizada a precios
                    increíbles.
                </p>
            </div>
